...
## Quick Reference: Imperative Rules

1. List your assumptions explicitly before writing code; ask before acting on ambiguous ones.
2. If you cannot ask (non-interactive context or no response), make the smallest reasonable assumption, state it explicitly, and proceed.
3. Make the smallest change that satisfies a verifiable goal.
4. Match the existing code style
5. Add abstraction only when two or more concrete variants already exist in the codebase or specification.
6. Write a failing test (or define a verifiable acceptance criterion) before fixing a bug or adding a feature.
7. Verify edge cases related to the stated goal; do not expand scope beyond that goal.
8. Commit after each unit that compiles and passes existing tests.
9. Keep commits small, descriptive, and in Conventional Commits format (all lowercase).
10. Work on `main` for solo/personal work; create a branch when the project uses a shared or production workflow.

## Principle Precedence

When principles conflict, apply them in this order:

1. **Surface assumptions first** — ask (or state) before acting.
2. **Solve the root cause architecturally** — step back, list known facts, identify the real boundary or invariant, and fix that; do not patch symptoms.
3. **Make the smallest change that satisfies a verifiable goal** — stop there.
4. **Match existing style** — do not drift.
5. **Commit** — keep rollback cheap.
...
Before writing code, separate **observed symptoms** from **known facts**:

- What did we directly observe in logs, tests, screenshots, or user reports?
- Which component owns the meaning of the failing behavior?
- What invariant should hold for all similar cases, not just this one?
- What is the smallest verifiable change that restores that invariant?

Prefer architecture-level fixes over local symptom patches. A good fix moves the
right responsibility into the right layer, names the invariant, and should improve
a class of related cases without app-specific, language-specific, or one-off
conditionals unless the platform boundary itself requires them.

After every code change, pause and self-review before moving on:

- Is this change truly necessary for the stated goal?
- Does it work naturally with the existing architecture and ownership boundaries?
- Does it solve the root cause, or is it merely hiding the symptom?
- Did it add the least complexity needed to restore the invariant?
- Would the same rule still make sense for the next similar case?

If the answer is unclear or "no", revisit the approach, reduce the diff, or try
again from the facts. Do not keep a change just because it appears to work.
...
(If I can't get a response, I'll assume: API endpoint returning paginated JSON
for the current user only — the smallest safe interpretation. I'll state this
...
**When to add complexity:** Add abstraction only when two or more concrete variants exist in the current codebase or specification — not for hypothetical future variants.
...
| Surgical Changes    | Reformats quotes, adds type hints while fixing bug | Only change lines that fix the reported issue; verify related edge cases via tests, not extra code |
...
- Commit after each unit that compiles and passes existing tests — typically one logical change such as a bug fix, a new function, or a refactor of a single component.
- Keep commits small and descriptive.
- Use [Conventional Commits](https://www.conventionalcommits.org/) format.
- Commit messages must be all lowercase.
- Stay on `main` for solo/personal work; create a branch when the project uses a shared or production workflow.
- The purpose of frequent commits is operational safety, not process overhead.
- Never squash commits that fix bugs or add features — keep the history clear and traceable.
...
Remember we want this to be first principles approach, separation of concerns, and stick with our core development principles. We build foundation to be able to extend easily later on. Simplicity is the ultimate sophistication. We will not cover or patch the issue, but only fix at the root cause long term, best practice.
